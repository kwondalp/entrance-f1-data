...
The remaining-stats continuation accepts five further DOB corrections (784 accepted dates; 17 of 29 disputes resolved, 12 retained), fixes POR/PUR and two duplicate driver identities, and binds 3,098 season nationality intervals plus 29 event observations. Mazepin 2021 rounds 1-21 remain neutral-flag classification questions, not missing Russian nationality. Five additional metrics are complete, but full reference comparison reopens two previously completed leader-age metrics: the original cohort is now 36/50 complete and 14 incomplete. Published leader charts lack 365 events; all three leader-age metrics expose that precise limitation. Full podium comparison covers 438 rows (436 matches, two accepted Menditeguy DOB differences), plus 120 country-reference rows with cutoff/population/calendar differences documented. Normal regeneration and controlled new-event/new-driver replay retain and recalculate the fixes. Candidate schema/hash checks pass for 169 metrics and 69,465 rows; focused Tools 65 unique passes, 6 skips and 2 unchanged production gates; Web 124 passes and one 932-file build. No production approval, main merge or deployment.

The Web candidate review and single audit ledger supersede the previous 33/17 demographic status with the evidence-corrected 36/14 status. Exact DOB alternatives and source-quality reasons are retained in Tools `fixtures/stats-dob-dispute-review-20260917.json`; machine-readable validation is `docs/stats-office-validation-20260917.json`. Original source archives were verified locally; newly fetched evidence is labelled as new collection. No indispensable home-only file was required.
