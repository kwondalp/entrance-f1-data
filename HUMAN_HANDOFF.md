...
## Production recovery and development continuation, 2026-09-14

Production Race recovery is complete: the real publisher activated the Spanish GP race, all 23 driver and 11 constructor championship totals, and related Stats. Kimi Antonelli changed from 267 to 292 points and seven to eight wins; Mercedes changed from 468 to 503 points. Production desktop and 375-by-812 mobile checks covered Home, Results, both standings and Wins, including exact response hashes. A subsequent same-Race replay made no changes. Profile identity/fact evidence remains dated 2026-09-06; current statistics are independently bound through 2026-09-13.

The queued scheduled run failed closed because its event-time Tools checkout preceded the completed publication while Data resolved the newer state. Core checkout now resolves current main after queueing. Manual execution of that version passed. Corrected run 34819909414 was an actual schedule event created at 2026-09-14 07:52:47 UTC and completed successfully with the R14 Race already_published no-op and verified_preserved completion contracts. Public response hashes and production standings were rechecked afterwards. The temporary Codex observation monitor was paused after success; the production GitHub schedule remains active.

Development synchronization preserves earlier feature work and research artifacts. Current immutable files bind the actual published core bytes. An independent age population/event-selection audit passed all 25 age metrics within the accepted DOB set; source identity normalization remains shared. Broader additional-metric population completeness remains pending. The five unsupported Stats still lack sufficient nationality/DOB evidence, and the shared-driver backfill repair remains a development proposal at its original coverage.

Power race-seed replay re-measured actual R13 and R14 source inputs against current canonical events. Research now reaches R14. Qualifying coverage remains 98/260 (37.6923 percent), below the unchanged 50 percent gate; no model is selected or activated. The 2025 round 17 qualifying provider failure remains unresolved. New research stays manual-only. Its earlier zero-job push failures were reproduced as YAML parsing errors in the unquoted dependency-install command and corrected with a block scalar. Value again matches all 23 latest official point totals and produces a same-input no-op; salary definitions and existing operational publication policy are unchanged. Its new generator fingerprint remains an explicit review proposal.

Validation: Tools full suite 1096 passed, eight prerequisite-dependent skips; Web full suite 309 passed and the normal static production artifact built. Loading profile source/cutoff binding, mobile overflow, filter URL/reset, final Wins row keyboard access and Last Win UTC display were checked separately from production recovery evidence. No new independent raw-source audit or full all-route browser audit is claimed.

## Home continuation

See [docs/home-continuation.md](docs/home-continuation.md) for portable setup, retained evidence and the next development steps. The corrected real scheduled core run is now observed successful, separately from manual publication and manual no-op. Linux recovery CI passed 1092 Tools tests with 12 prerequisite-dependent skips, all 309 Web tests and the 344-file static build. Local caches and stashes are not transferred by Git; preserve them and reacquire only checksum-verified source inputs when needed.
