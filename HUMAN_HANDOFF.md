...
The queued scheduled run failed closed because its event-time Tools checkout preceded the completed publication while Data resolved the newer state. Core checkout now resolves current main after queueing. Manual execution of that version passed. The enabled schedule is separate from observed success of a corrected scheduled run, which remains to be confirmed.
