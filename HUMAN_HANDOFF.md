...
Continued the existing 472-metric audit without restarting it. Shared qualifying-stage, historical grid geometry and eligible-DOB calculations plus consumer evidence notices are repaired. Two false DOB blockers are resolved; 50 evidence blockers and four unsupported routes remain. Nine official DOB proposals and exact publication/profile review requirements are prepared but unapproved. Production release and main integration remain pending.

Identical-input regeneration matches all 553 package files. The changed front-row-duo oracle passes 514 rows and supersedes its earlier two-slot comparison; unchanged base and backfill audit evidence is reused. Tools final evidence totals 1191 passed, two unchanged approval/profile gates failed and 16 skipped after isolating Windows temporary-directory and Git path issues. The Power workflow assertion now tests its actual legitimate guard. Web full suite passes 357 tests, subsequent affected checks pass, and the 930-file HTTP artifact retains explicit incomplete-ranking notices and removes the two obsolete Stats panels. The single detailed ledger and exact review proposal are entrance-web/docs/stats-catalogue-audit-20260917.json and entrance-web/docs/stats-candidate-review-20260917.md. No unapproved DOB or historical nationality facts were admitted.
